...
JSONFormatter.prototype = {
  ...
  /**
   * Decode an HTML string encoded with htmlEncode
   */
  ...
  jsonToHTML: function(json, callback, uri) {
    var json_orig = this.htmlEncode(JSON.stringify(json));
    var output = '<pre id="json_unformatted" style="display:none">' + json_orig + '</pre>' +
        '<div id="json">' + this.valueToHTML(json, '<root>') + '</div>';
    if (callback) {
  ...
  toHTML: function(content, title) {
    return '<!DOCTYPE html>\n' +
      ...
      '<script type="text/javascript" src="' + addonData.url("jsonpath.js") + '"></script>' +
      '</head><body>' +
      '<div style="background-color:#626262; position: static; height:40px; color:white">' +
        '<div style="float:right; position:absolute; right:50px" />JSONpath expr ' +
            '<input type="text" size="40" id="jsonpath_expr" />' +
        '</div>' +
      '</div>' +
      content +
      '</body></html>';
